...
{
    [SerializeField] Pool[] ButterProjectPools;
    //[SerializeField] Pool[] playerProjectilePools;
    [SerializeField] Pool[] enemyProjectilePools;
    [SerializeField] Pool[] playerBulletsVfx;
    [SerializeField] Pool[] enemy;
    [SerializeField] Pool[] Droping;//掉落物
    ...
    void Awake()
    {
        dictionary = new Dictionary<GameObject, Pool>(); //存储不同数字球对象池的字典

        Initialize(ButterProjectPools);//初始化生成数字球的对象池
        Initialize(enemyProjectilePools);//初始化生成数字球的对象池
        Initialize(playerBulletsVfx);
        Initialize(enemy);
        Initialize(Droping);
    ...
    void OnDestroy()
    {
        CheckPoolSize(ButterProjectPools);
        //CheckPoolSize(playerProjectilePools);
        CheckPoolSize(enemyProjectilePools);
        CheckPoolSize(playerBulletsVfx);
        CheckPoolSize(enemy);
        CheckPoolSize(Droping);
    }
#endif
